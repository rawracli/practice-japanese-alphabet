...
export default function QuizHeader({
  ...
  return (
    <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 p-4 md:p-6 rounded-2xl border border-slate-800 bg-slate-950/40 glass-panel">
      {/* Quiz Mode Details */}
      <div className="flex items-center gap-3">
        <button
          onClick={onFinish}
          type="button"
          className="flex items-center justify-center p-2.5 rounded-xl border border-slate-800 bg-slate-900 hover:border-slate-700 text-slate-400 hover:text-slate-200 transition-all duration-200"
          title="Exit Practice"
        >
        ...
        <div>
          ...
          <span className="font-extrabold text-slate-200 text-sm md:text-base leading-tight">
            {modeName}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 md:gap-6">
        {/* Progress Counter */}
        <div className="text-center bg-slate-900/60 border border-slate-900 rounded-xl px-4 py-2">
        ...
        {/* Stopwatch Timer */}
        <div className="text-center bg-slate-900/60 border border-slate-900 rounded-xl px-4 py-2">
          <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider block">Time</span>
          <span className="font-mono text-sm md:text-base font-bold text-brand-accent">
            {formatTime(time)}
          </span>
        </div>

        {/* Streak Combo */}
        <div className={`text-center border rounded-xl px-4 py-2 transition-all duration-300 ${getStreakStyles()}`}>
          <span className="text-[10px] uppercase font-bold tracking-wider block opacity-70">Streak</span>
          <span className="text-sm md:text-base font-black flex items-center justify-center gap-1">
            {streak > 0 && (
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          type="button"
          className="flex items-center justify-center p-2.5 rounded-xl border border-slate-800 bg-slate-900 hover:border-slate-700 text-slate-400 hover:text-slate-200 transition-all duration-200 cursor-pointer"
